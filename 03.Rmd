...
\end{equation}
\begin{equation}
R_{u} = np + z_{\lbrack 1 - \alpha / 2 \rbrack} \bullet \sqrt{np(1 - p)} + 0.5
(\#eq:3-17)
\end{equation}
...
x_l <- x[R_l]
x_u <- x[R_u]
nonparametric <- paste0("nonparametric: ", x_l, " ≤ ", median_x,  " ≤ ", x_u)
...
parametric <- paste0("parametric: ", y_l, " ≤ ", GM_x, " ≤ ", y_u)

nonparametric
parametric
```

Either of intervals is reasonable. The logs of the data still retain some skewness, so the nonparametric interval may be more realistic. The choice would depend on whether the assumption of lognormality was believed. 
...
symmetric <- paste0("Symmetric: ", x_l, " ≤ ", mean_x, " ≤ ", x_u)
...
sol_1 <- paste0(PI_l, " to ", PI_u, " gallons/min/foot.")
sol_2 <- "Includes 0.85, so same distribution."

low_idx <- (ALPHA / 2) * (N + 1)
up_idx <- (1 - (ALPHA / 2)) * (N + 1)

sol_3 <- "Nonparametric 95% prediction interval: "
sol_4 <- paste0("X_", low_idx, " to ", "X_", up_idx)
cat(sol_0, sol_1, sol_2, sol_3, sol_4, sep = "\n")
```
...
sol_3 <- paste0("median:    R_l = ", R_l, ", R_u = ", R_u)
sol_4 <- paste0(x[R_l], " to ", x[R_u])
...
sol_0 <- paste0("The 10th percentile = ", x_10, " cfs")
sol_1 <- "A one-sided 95% confidence interval for the 90th percentile (an upper confidence limit"
sol_2 <- "to insure that the intake does not go dry) is found using the large-sample"
sol_3 <- "approximation of equation 3.17: "
sol_4 <- paste0("R_u = ", R_u)
sol_5 <- paste0("The 46th ranked point is the upper CI, or ", x_u ," cfs")
cat(sol_0, sol_1, sol_2, sol_3, sol_4, sol_5, sep = "\n")
```
